...
Three fixes from the review were applied (rolling-24h window helper `stableHoursAgoIso`;
Billing "MTD" now month-filtered; worker-name demo leak via `maskWorkerName`).
Remaining findings, to triage:
...
- leak-check CI case-sensitivity is intentional, not a bug: it catches lowercase `3211labs` (the real `3211labs.com` domain) while tolerating the public GitHub org slug `3211Labs`, which must appear in clone URLs / the About card. An earlier pass made it case-insensitive; reverted, since that flagged the repo's own org slug. If the repo should be fully brand-detached, scrub the org slug from clone URLs instead (owner decision).
